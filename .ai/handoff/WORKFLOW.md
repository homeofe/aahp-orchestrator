# aahp-orchestrator: Autonomous Multi-Agent Workflow
...
|-------|-------|------|---------------|
| Researcher | perplexity/sonar-pro | Researcher | VS Code extension API docs, Marketplace requirements |
| Architect | claude-opus | Architect | Extension architecture, CI/CD design |
| Implementer | claude-sonnet | Implementer | TypeScript code, tests, commits |
| Reviewer | gpt-4o / copilot | Reviewer | Security review, edge cases, CSP compliance |
...
Does:    Researches VS Code extension APIs relevant to the task
         Checks whether similar solutions already exist in src/
...
Reads:   ADR from LOG.md
         ...
         src/ files relevant to the task

Does:    Creates feature branch: git checkout -b feat/<scope>-<name>
         Writes code + tests
         Runs: npm run compile && npm run lint
...
Researcher - "Were all task items fulfilled? Any VS Code Marketplace policy concerns?"
...
STATUS.md:       Update changed system state (verified / assumed / untested)
...
Notify:  Project owner - only on fully completed tasks, not phase transitions
         Format: "T-001 done - Branch: feat/... - Compile: pass - Lint: pass"
```
...
| Research VS Code extension APIs | Mutate .ai/handoff/ files of OTHER repos |
| Make architecture decisions | Perform VS Code Marketplace publish without human approval |
...
1. Read `DASHBOARD.md`, take the top task where `Ready? = Ready`
...
- Mark task as `untested` in `STATUS.md`
...
## VS Code Extension Specific Notes

- Use `vscode.workspace.findFiles()` not `fs.readdir()` for workspace file discovery
- Webview scripts must be loaded via `panel.webview.asWebviewUri()` - no inline scripts
- All VS Code API calls must be inside try/catch - extension must not crash VS Code
- Test with `code --extensionDevelopmentPath` before packaging
- `vsce package` requires `README.md` and valid `publisher` in package.json
